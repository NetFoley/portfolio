...
func updateDirty(length) -> void:
	dirtyLength = length
func updateClean(length) -> void:
	cleanLength = length

func _process_custom_fx(char_fx):
	# Get parameters, or use the provided default value if missing.
	var clear_time = char_fx.env.get("clean", cleanLength)
	var dirty_time = char_fx.env.get("dirty", dirtyLength)
	var text_span = char_fx.env.get("span", 1000)
